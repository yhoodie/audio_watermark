...
async function queryVideoTask(taskId: string): Promise<VideoTaskResponse> {
  ...
  const response = await fetch(
      `https://app-dyjft4hsvfgh-api-Q9KWPzO1Eg69-gateway.appmiaoda.com/doubao/v3/contents/generations/tasks/${taskId}`,
    {
      method: "GET",
      headers: {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const upstreamResponse = await fetch(
      "https://app-dyjft4hsvfgh-api-nYWN4kMJ1v7L-gateway.appmiaoda.com/doubao/v3/contents/generations/tasks",
      {
        method: "POST",
        headers: {
